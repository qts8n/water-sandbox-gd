...
static func create_vec3_zeros(data_length: int) -> StorageBufferUniform:
	var data = []
	for _it in range(data_length):
		data.append(Vector3.ZERO)
	return StorageBufferUniform.create(PackedVector3Array(data).to_byte_array())
